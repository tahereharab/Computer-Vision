# -*- coding: utf-8 -*-
"""
Created on Fri Feb 23 18:07:34 2018

@author: Tahereh
"""
from wan_getCandidates import get_items
...
def getItemsFromAPI(method):
    ...
    for row in csv_file:
        if row:
            ...
            P1list = []  
            #print(P1, P2, P3)
            P1items  = get_items(P1,limit,api_key,api_url_base,True)
            #print(P1items)
            if len(P1items) >= 10:
                length = 10
            else:
            ...
            #######################
            P2list = []        
            P2items  = get_items(P2,limit,api_key,api_url_base,True)    
            if len(P2items) >= 10:
                length = 10
            else:
                length = len(P2items)
            ...
            #######################
            P3list = []        
            P3items  = get_items(P3,limit,api_key,api_url_base,True)       
            if len(P3items) >= 10:
                length = 10
            else:
                length = len(P3items)
            ...
            #word2vec
            if method == 'w2v':
                with open('wan_GTProps_props_w2v.csv','a') as file:
                    writer = csv.writer(file)
                    writer.writerow([source, target,P1, P1itemsStr, P2, P2itemsStr, P3, P3itemsStr])
         
            else:            
            #wordnet   
                with open('wan_GTProps_props_wordnet.csv','a') as file:
                    writer = csv.writer(file)
                    writer.writerow([source, target,P1, P1itemsStr, P2, P2itemsStr, P3, P3itemsStr])  
...
if __name__ == '__main__':
    ...
    #getRankForExactCandidates('wordnet')    
    #getRankForSimilarCandidates('union') 
